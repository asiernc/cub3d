...
/*   Updated: 2024/07/18 16:33:31 by molasz-a         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	set_direction(t_render *render)
{
	if(render->ray_dir.x < 0)
	{
		render->step.x = -1;
		render->side_dist.x =
			(render->player.x - render->map.x) * render->delta_dist.x;
	}
	else
	{
		render->step.x = 1;
		render->side_dist.x =
			(render->map.x + 1.0 - render->player.x) * render->delta_dist.x;
	}
	if(render->ray_dir.y < 0)
	{
		render->step.y = -1;
		render->side_dist.y =
			(render->player.y - render->map.y) * render->delta_dist.y;
	}
	else
	{
		render->step.y = 1;
		render->side_dist.y =
			(render->map.y + 1.0 - render->player.y) * render->delta_dist.y;
	}
}
...
static void	draw_line(t_cub3d *cub3d, int x, int draw[2], unsigned int color)
{
	int	y;
